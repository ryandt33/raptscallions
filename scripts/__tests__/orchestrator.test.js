...
function getNextState(currentState, rejected = false) {
    if (rejected) {
...
describe("Orchestrator", () => {
    describe("State Transitions", () => {
        ...
        describe("Rejection Paths", () => {
            ...
            it("should reject INTEGRATION_TESTING to INTEGRATION_FAILED", () => {
                expect(getNextState("INTEGRATION_TESTING", true)).toBe("INTEGRATION_FAILED");
            });
    ...
    describe("Epic Management", () => {
        ...
        describe("Epic Ordering in Auto Mode", () => {
            ...
            function sortTasksForAutoMode(tasks) {
            ...
            it("should prioritize incomplete E01 tasks over E02 tasks", () => {
                ...
                // E01 is not complete
                expect(isEpicComplete(tasks.filter((t) => t.epic === "E01"))).toBe(false);
            ...
            it("should only start E02 after all E01 tasks are DONE", () => {
                ...
                expect(isEpicComplete(tasks.filter((t) => t.epic === "E01"))).toBe(true);
                const ready = getReadyTasks(tasks);
                const sorted = sortTasksForAutoMode(ready);
        ...
        describe("Task Resolution Order Simulation", () => {
            ...
            function simulateAutoMode(initialTasks) {
                ...
                while (iterations < maxIterations) {
                    ...
                    const completedIds = getCompletedTaskIds(tasks);
                    const ready = tasks.filter((t) => canStartTask(t, completedIds) &&
                        t.workflow_state !== "DONE" &&
                        !t.breakpoint);
                    if (ready.length === 0)
                        break;
            ...
            it("should respect cross-epic dependencies", () => {
                ...
                expect(order.indexOf("E01-T001")).toBeLessThan(order.indexOf("E02-T001"));
                expect(order.indexOf("E01-T002")).toBeLessThan(order.indexOf("E02-T001"));
            });
    ...
    describe("Workflow Consistency", () => {
        it("should have matching states in STATE_COMMANDS and STATE_TRANSITIONS", () => {
            const commandStates = Object.keys(STATE_COMMANDS);
            const transitionStates = Object.keys(STATE_TRANSITIONS);
        ...
        it("should only have DONE, INTEGRATION_FAILED, and PR_FAILED as terminal states", () => {
            const terminalStates = Object.entries(STATE_TRANSITIONS)
                ...
                .map(([state, _]) => state);
            expect(terminalStates.sort()).toEqual(["DONE", "INTEGRATION_FAILED", "PR_FAILED"].sort());
        });
