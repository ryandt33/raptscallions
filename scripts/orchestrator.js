...
const WORKFLOW_CONFIG_PATH = path.join(ROOT_DIR, "backlog", "docs", ".workflow", "config.yaml");
...
// DEPENDENCY CHECKING
// ============================================================================
function getCompletedTaskIds(tasks) {
    return new Set(tasks.filter((t) => t.workflow_state === "DONE").map((t) => t.id));
}
function canStartTask(task, completedIds) {
    if (task.workflow_state === "DONE")
        return false;
...
function getReadyTasks(tasks) {
    ...
    return tasks.filter((t) => canStartTask(t, completedIds) &&
...
function getBlockedTasks(tasks) {
    const completedIds = getCompletedTaskIds(tasks);
    return tasks
        .filter((t) => t.workflow_state !== "DONE" && !canStartTask(t, completedIds))
        .map((t) => ({
        task: t,
...
// ============================================================================
async function runClaudeCode(command, taskId, _freshContext = false) {
    const args = [
...
function shouldSkipUIReview(task, state) {
...
function getNextState(currentState, rejected = false, task) {
    if (rejected) {
        // Rejection paths
        // Note: Rejections go to TESTS_READY (not IMPLEMENTING) so the implement command runs
        if (currentState === "UX_REVIEW")
            return "ANALYZING";
        if (currentState === "PLAN_REVIEW")
            return "ANALYZING";
        if (currentState === "UI_REVIEW")
            return "TESTS_READY";
        if (currentState === "CODE_REVIEW")
            return "TESTS_READY";
        if (currentState === "QA_REVIEW")
            return "TESTS_READY";
        if (currentState === "INTEGRATION_TESTING")
...
async function runWorkflowStep(task, _config) {
    ...
    if (shouldSkipUIReview(task, currentState)) {
        ...
        if (nextState) {
            logStep("SKIP", `${task.id}: Skipping ${currentState} (backend-only task) → ${nextState}`);
    ...
    if (task.skip_github && currentState === "PR_READY") {
        logWarning(`Task ${task.id} has skip_github=true. Skipping GitHub automation.`);
        logInfo(`Please manually commit and create PR for ${task.id}, then set workflow_state to DONE.`);
...
async function runFullWorkflow(taskId, config) {
    ...
    if (!canStartTask(task, completedIds)) {
        const blockedBy = (task.depends_on || []).filter((d) => !completedIds.has(d));
    ...
    while (continueWorkflow && iterations < maxIterations) {
        ...
        if (config.default_breakpoints.includes(currentTask.workflow_state) &&
            !currentTask.breakpoint) {
            logWarning(`Default breakpoint at ${currentTask.workflow_state}. Use --force to continue.`);
            break;
        }
...
function showStatus(taskId) {
    ...
    if (taskId) {
        ...
        const completedIds = getCompletedTaskIds(tasks);
        const depsStatus = (task.depends_on || []).map((d) => `${completedIds.has(d) ? "✅" : "⏳"} ${d}`);
        ...
        if (task.spec_file)
            log(`Spec:  ${task.spec_file}`);
        if (task.test_files?.length)
            log(`Tests: ${task.test_files.join(", ")}`);
        if (task.code_files?.length)
            log(`Code:  ${task.code_files.join(", ")}`);
    }
    else {
        // Show board
...
function showNext() {
    ...
    const breakpointTasks = tasks.filter((t) => t.breakpoint && t.workflow_state !== "DONE");
...
function setBreakpoint(taskId, enable) {
    const tasks = loadAllTasks();
    const task = tasks.find((t) => t.id === taskId);
    if (!task) {
        logError(`Task not found: ${taskId}`);
...
function setSkipGitHub(taskId, enable) {
    const tasks = loadAllTasks();
    const task = tasks.find((t) => t.id === taskId);
    if (!task) {
        logError(`Task not found: ${taskId}`);
...
function isEpicComplete(epicTasks) {
    return epicTasks.length > 0 && epicTasks.every((t) => t.workflow_state === "DONE");
}
function getCompletedEpics(tasks) {
...
// MAIN
// ============================================================================
async function runAutoMode(config, continuous = false) {
    log("");
    log(`${colors.bold}═══════════════════════════════════════════════════════════════${colors.reset}`);
    log(`${colors.bold}  ${continuous ? "Continuous" : "Auto"} Mode - Running All Ready Tasks${colors.reset}`);
    log(`${colors.bold}═══════════════════════════════════════════════════════════════${colors.reset}`);
    log("");
    let tasksProcessed = 0;
    let epicsCompleted = 0;
    let continueLoop = true;
    const reviewedEpics = new Set(); // Track which epics we've already reviewed
    while (continueLoop) {
        const tasks = loadAllTasks();
        const ready = getReadyTasks(tasks);
        ...
        if (ready.length === 0) {
            ...
            if (unreviewedEpics.length > 0) {
                ...
                for (const epicId of unreviewedEpics) {
                    ...
                    const reviewResult = await runClaudeCode("epic-review", `${epicId} --create --threshold high`, false);
    ...
    log("");
    log(`${colors.bold}═══════════════════════════════════════════════════════════════${colors.reset}`);
    logSuccess(`${continuous ? "Continuous" : "Auto"} mode complete. Processed ${tasksProcessed} task(s), ${epicsCompleted} epic(s).`);
    log(`${colors.bold}═══════════════════════════════════════════════════════════════${colors.reset}`);
}
async function main() {
    ...
    switch (command) {
        case "run": {
            const taskId = args[1];
            const hasContinuous = args.includes("--continuous") || args.includes("-c");
        ...
        default: {
            ...
            log("  pnpm workflow:status [task-id]           Show task board or specific task");
            log("  pnpm workflow:next                       Show next available tasks");
