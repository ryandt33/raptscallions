...
describe("buildAbility", () => {
    ...
    describe("system_admin", () => {
        ...
        it("should bypass all other permissions when system_admin", () => {
            ...
            expect(ability.can("delete", subject("Tool", { createdBy: "other-user" }))).toBe(true);
    ...
    describe("group_admin", () => {
        ...
        it("should allow managing classes in their groups", () => {
            ...
            expect(ability.can("create", subject("Class", { groupId: "group-2" }))).toBe(false);
        });
        ...
        it("should allow managing assignments in their groups", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("create", subject("Assignment", { groupId: "group-1" }))).toBe(true);
            expect(ability.can("manage", subject("Assignment", { groupId: "group-1" }))).toBe(true);
            expect(ability.can("create", subject("Assignment", { groupId: "group-2" }))).toBe(false);
        });
    ...
    describe("teacher", () => {
        ...
        it("should allow managing their own tools", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("read", subject("Tool", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("update", subject("Tool", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Tool", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Tool", { createdBy: "other-user" }))).toBe(false);
        });
        ...
        it("should allow managing their own assignments", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("read", subject("Assignment", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("update", subject("Assignment", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Assignment", { createdBy: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Assignment", { createdBy: "other-user" }))).toBe(false);
        });
        ...
        it("should allow reading sessions for their tools", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("read", subject("Session", { toolCreatedBy: mockUser.id }))).toBe(true);
            expect(ability.can("read", subject("Session", { toolCreatedBy: "other-user" }))).toBe(false);
        });
    ...
    describe("student", () => {
        it("should allow reading assigned tools", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("read", subject("Tool", { assignedTo: mockUser.id }))).toBe(true);
            expect(ability.can("read", subject("Tool", { assignedTo: "other-user" }))).toBe(false);
        });
        it("should allow reading assigned assignments", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("read", subject("Assignment", { assignedTo: mockUser.id }))).toBe(true);
            expect(ability.can("read", subject("Assignment", { assignedTo: "other-user" }))).toBe(false);
        });
        it("should allow managing own sessions", () => {
            ...
            const ability = buildAbility({ user: mockUser, memberships });
            // Assert
            expect(ability.can("create", subject("Session", { userId: mockUser.id }))).toBe(true);
            expect(ability.can("read", subject("Session", { userId: mockUser.id }))).toBe(true);
            expect(ability.can("update", subject("Session", { userId: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Session", { userId: mockUser.id }))).toBe(true);
            expect(ability.can("delete", subject("Session", { userId: "other-user" }))).toBe(false);
        });
...
describe("canManageGroupHierarchy", () => {
    ...
    it("should allow managing exact group", () => {
        ...
        const ability = buildAbility({ user: mockUser, memberships });
        const userGroupPaths = [
            { groupId: "group-1", path: "district.school1" },
        ];
        // Act
        const result = canManageGroupHierarchy(ability, "group-1", userGroupPaths, "district.school1");
        // Assert
    ...
    it("should allow managing descendant groups", () => {
        ...
        // Act
        const result = canManageGroupHierarchy(ability, "group-2", userGroupPaths, "district.school1.dept_math");
        // Assert
        expect(result).toBe(true);
    });
    it("should deny managing sibling groups", () => {
        ...
        // Act
        const result = canManageGroupHierarchy(ability, "group-2", userGroupPaths, "district.school2");
        // Assert
    ...
    it("should deny managing parent groups", () => {
        ...
        // Act
        const result = canManageGroupHierarchy(ability, "group-2", userGroupPaths, "district.school1");
        // Assert
        expect(result).toBe(false);
    });
    it("should handle multiple managed group paths", () => {
        ...
        // Act & Assert
        expect(canManageGroupHierarchy(ability, "group-3", userGroupPaths, "district.school1.dept_math")).toBe(true);
        expect(canManageGroupHierarchy(ability, "group-4", userGroupPaths, "district.school2.dept_science")).toBe(true);
        expect(canManageGroupHierarchy(ability, "group-5", userGroupPaths, "district.school3.dept_arts")).toBe(false);
    });
    ...
    it("should handle deeply nested hierarchy", () => {
        ...
        expect(canManageGroupHierarchy(ability, "group-2", userGroupPaths, "district.school1.dept_math.grade_9.section_a")).toBe(true);
    });
    it("should use ability check as first priority", () => {
        ...
        const userGroupPaths = [];
        // Act
        const result = canManageGroupHierarchy(ability, "any-group", userGroupPaths, "any.path");
