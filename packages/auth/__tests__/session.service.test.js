...
describe("SessionService", () => {
    ...
    describe("validate", () => {
        ...
        it("should throw UnauthorizedError when session validation throws", async () => {
            // Arrange
            vi.mocked(lucia.validateSession).mockRejectedValue(new Error("Database error"));
            // Act & Assert
            await expect(service.validate("malformed-id")).rejects.toThrow(UnauthorizedError);
            await expect(service.validate("malformed-id")).rejects.toThrow("Invalid session");
        });
        it("should handle shared device session context", async () => {
            ...
            expect(result.session?.expiresAt.getTime()).toBeLessThan(Date.now() + 3 * 60 * 60 * 1000); // Less than 3 hours
        });
    });
    describe("create", () => {
        ...
        it("should create session with future expiration", async () => {
            ...
            expect(result.expiresAt.getTime()).toBeLessThanOrEqual(Date.now() + 31 * 24 * 60 * 60 * 1000);
        });
        it("should throw Error when session creation fails", async () => {
            // Arrange
            vi.mocked(lucia.createSession).mockRejectedValue(new Error("Database error"));
            // Act & Assert
            await expect(service.create("user-fail")).rejects.toThrow("Failed to create session");
        });
    ...
    describe("invalidate", () => {
        ...
        it("should not throw when session does not exist", async () => {
            // Arrange
            vi.mocked(lucia.invalidateSession).mockRejectedValue(new Error("Session not found"));
            // Act & Assert - should not throw
            await expect(service.invalidate("non-existent-session")).resolves.toBeUndefined();
        });
        it("should silently handle already invalidated session", async () => {
            // Arrange
            vi.mocked(lucia.invalidateSession).mockRejectedValue(new Error("Session already invalid"));
            // Act & Assert
            await expect(service.invalidate("already-invalid")).resolves.toBeUndefined();
        });
    ...
    describe("invalidateUserSessions", () => {
        ...
        it("should throw Error when user session invalidation fails", async () => {
            // Arrange
            vi.mocked(lucia.invalidateUserSessions).mockRejectedValue(new Error("Database error"));
            // Act & Assert
            await expect(service.invalidateUserSessions("user-fail")).rejects.toThrow("Failed to invalidate user sessions");
        });
        it("should handle user with no active sessions", async () => {
            // Arrange
            vi.mocked(lucia.invalidateUserSessions).mockResolvedValue();
            // Act & Assert
            await expect(service.invalidateUserSessions("user-no-sessions")).resolves.toBeUndefined();
        });
    ...
    describe("createBlankSessionCookie", () => {
        it("should create blank cookie for logout", () => {
            ...
            vi.mocked(lucia.createBlankSessionCookie).mockReturnValue(mockBlankCookie);
            // Act
    ...
    describe("Edge Cases", () => {
        it("should handle empty session ID gracefully", async () => {
            // Arrange
            vi.mocked(lucia.validateSession).mockRejectedValue(new Error("Invalid session ID"));
            // Act & Assert
            await expect(service.validate("")).rejects.toThrow(UnauthorizedError);
        });
        it("should handle malformed session ID", async () => {
            // Arrange
            vi.mocked(lucia.validateSession).mockRejectedValue(new Error("Malformed session ID"));
            // Act & Assert
            await expect(service.validate("malformed!!!")).rejects.toThrow(UnauthorizedError);
        });
