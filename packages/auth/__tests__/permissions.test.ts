...
describe("permissionMiddleware", () => {
  ...
  beforeEach(async () => {
    ...
    const mockSessionMiddleware = fp(async (fastify) => {
      fastify.addHook("onRequest", async (request) => {
        ...
        (request as any).session = mockUser ? { id: "session-123" } : null;
      });
  ...
  describe("onRequest hook", () => {
    ...
    it("should build ability from user memberships for authenticated requests", async () => {
      ...
      // Mock database query
      vi.mocked(db.query.groupMembers.findMany).mockResolvedValue(
        mockMemberships
      );
  ...
  describe("checkResourcePermission decorator", () => {
    ...
    it("should return true when resource permission granted", async () => {
      ...
      app.get("/check-resource", async (request) => {
        ...
        const canDelete = app.checkResourcePermission(
          request.ability,
          "delete",
          "Tool",
          tool
    ...
    it("should check resource-specific conditions", async () => {
      ...
      app.get("/check-resource", async (request) => {
        ...
        const canManageInGroup = app.checkResourcePermission(
          request.ability,
          "manage",
          "User",
          userInGroup
        );
        const canManageOtherGroup = app.checkResourcePermission(
          request.ability,
          "manage",
          "User",
          userInOtherGroup
  ...
  describe("getGroupPaths decorator", () => {
    ...
    it("should fetch and return group paths", async () => {
      ...
      vi.mocked(db.query.groups.findMany).mockResolvedValue(mockGroups as any);
    ...
    it("should handle full permission flow for teacher creating tool", async () => {
      ...
      app.post(
        ...
        async (request) => {
          ...
          const canCreate = app.checkResourcePermission(
            request.ability,
            "create",
            "Tool",
            tool
  ...
  describe("edge cases", () => {
    it("should handle database query failures gracefully", async () => {
      ...
      vi.mocked(db.query.groupMembers.findMany).mockRejectedValue(
    ...
    it("should handle user with deleted group memberships", async () => {
      ...
      const mockMemberships: GroupMember[] = [];
