...
describe("oauth", () => {
    ...
    describe("googleOAuthClient", () => {
        it("should initialize Google client with correct config", async () => {
            ...
            // Assert
            expect(Google).toHaveBeenCalledWith("test-google-client-id", "test-google-secret", "http://localhost:3000/auth/google/callback");
        });
    ...
    describe("microsoftOAuthClient", () => {
        it("should initialize Microsoft client with correct config", async () => {
            ...
            expect(MicrosoftEntraId).toHaveBeenCalledWith("common", // Tenant ID
            "test-microsoft-client-id", "test-microsoft-secret", "http://localhost:3000/auth/microsoft/callback");
        });
    ...
    describe("requireMicrosoftOAuth", () => {
        ...
        it("should throw AppError with 503 when Microsoft not configured", async () => {
            ...
            const { requireMicrosoftOAuth } = await import("../oauth.js");
            // Act & Assert
            expect(() => requireMicrosoftOAuth()).toThrow("Microsoft OAuth not configured");
            try {
