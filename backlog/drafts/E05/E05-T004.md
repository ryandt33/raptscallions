...
Implement local filesystem storage backend as fallback for development and self-hosted deployments without cloud storage. Files stored in organized directory structure with token-based download URLs.

## Acceptance Criteria

### Core Implementation
- [ ] AC1: LocalStorageBackend class implements IStorageBackend interface
- [ ] AC2: Files stored in LOCAL_STORAGE_PATH (default: ./storage/uploads)
- [ ] AC3: Directory structure: {basePath}/{groupId}/{year}/{month}/{uuid}.{ext}
- [ ] AC4: upload() creates directories if they don't exist (recursive mkdir)
- [ ] AC5: upload() writes buffer to disk with atomic write (tmp file + rename)
- [ ] AC6: download() reads file from disk, throws NotFoundError if missing
- [ ] AC7: delete() removes file from disk (soft delete handled in DB)
- [ ] AC8: exists() checks file existence using fs.access()
- [ ] AC9: getSignedUrl() returns token-based URL (not S3-style pre-signed)

### Token-Based URLs
- [ ] AC10: Generate JWT token containing storageKey and expiration
- [ ] AC11: Token signed with SESSION_SECRET from config
- [ ] AC12: Token valid for configured duration (default: 900 seconds)
- [ ] AC13: Download route validates token and serves file
- [ ] AC14: Set correct Content-Type header from file mime type
- [ ] AC15: Set Content-Disposition header for downloads

### Error Handling
- [ ] AC16: Throws StorageError if directory creation fails
- [ ] AC17: Throws StorageError if disk write fails
- [ ] AC18: Throws NotFoundError if file doesn't exist on download
- [ ] AC19: Throws StorageError if disk is full (ENOSPC)
- [ ] AC20: Logs detailed error context (path, operation, underlying error)

### Docker Integration
- [ ] AC21: Volume mount configured in docker-compose.yml
- [ ] AC22: Storage directory persists across container restarts
- [ ] AC23: .dockerignore excludes storage/uploads from image
- [ ] AC24: README documents volume setup for production

### Testing
- [ ] AC25: Unit tests verify all IStorageBackend methods
- [ ] AC26: Integration tests verify file persistence
- [ ] AC27: Tests verify token generation and validation
- [ ] AC28: Tests verify directory creation and cleanup
- [ ] AC29: Tests verify error handling for disk full, permissions
- [ ] AC30: Test coverage >80%
