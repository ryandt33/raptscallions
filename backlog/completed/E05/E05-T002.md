---
id: "E05-T002"
title: "Storage backend abstraction and configuration [RETIRED - See subtasks]"
status: "cancelled"
priority: "critical"
labels: [backend, infrastructure, architecture, retired]
assignee: ""
...
# E05-T002: Storage backend abstraction and configuration [RETIRED]

## ⚠️ Task Retired - See Subtasks

This task has been split into focused subtasks to address:
1. **Extensibility concern**: Hardcoded backend enum prevented custom storage backends
2. **Implementation focus**: Task contained ~300 lines of prescriptive code instead of outcome-focused requirements

**Replacement tasks:**
- **[E05-T002a](E05-T002a.md)**: Storage backend interface and extensible plugin system
- **[E05-T002b](E05-T002b.md)**: Storage configuration system with validation
...
1. **Hardcoded backend enum** (AC11): Limited to 5 backends ('local', 's3', 'azure', 'gcs', 'aliyun') - not extensible for custom backends
2. **Implementation details**: 300+ lines of TypeScript code prescribing exact implementation
3. **Too large**: 26 acceptance criteria covering interface, config, factory, errors, and testing
4. **Implementation-focused AC**: Criteria specified classes, methods, patterns instead of behaviors
...
| ---- | ----- | ----- | ----- |
| 2026-01-15 | RETIRED | pm | Split into E05-T002a (plugin system) and E05-T002b (config) to address extensibility and remove implementation code |
