...
describe("OAuth Routes Integration", () => {
    ...
    beforeAll(async () => {
        ...
        mockMicrosoftClient = {
            ...
            createAuthorizationURL: vi.fn().mockResolvedValue(new URL("https://login.microsoftonline.com/common/oauth2/v2.0/authorize?state=mock-state-abc123")),
            validateAuthorizationCode: vi.fn().mockResolvedValue({
                accessToken: () => "mock-access-token",
                accessTokenExpiresAt: () => new Date(Date.now() + 3600000),
                refreshToken: () => null,
    ...
    describe("GET /auth/google", () => {
        ...
        it("should call Google client with correct scopes", async () => {
            // Arrange
            mockGoogleClient.createAuthorizationURL.mockResolvedValue(new URL("https://accounts.google.com/o/oauth2/v2/auth"));
            // Act
            await app.inject({
                method: "GET",
                url: "/auth/google",
            ...
            // Assert
            expect(mockGoogleClient.createAuthorizationURL).toHaveBeenCalledWith("mock-state-abc123", "mock-code-verifier-123", ["email", "profile"]);
        });
    });
    ...
    describe("GET /auth/microsoft", () => {
        it("should set oauth_state cookie and redirect to Microsoft", async () => {
            // Arrange
            mockMicrosoftClient.createAuthorizationURL.mockResolvedValue(new URL("https://login.microsoftonline.com/common/oauth2/v2.0/authorize?state=mock-state-abc123"));
            // Act
            const response = await app.inject({
                method: "GET",
                url: "/auth/microsoft",
        ...
        it("should call Microsoft client with correct scopes", async () => {
            // Arrange
            mockMicrosoftClient.createAuthorizationURL.mockResolvedValue(new URL("https://login.microsoftonline.com/common/oauth2/v2.0/authorize"));
            // Act
            await app.inject({
                method: "GET",
                url: "/auth/microsoft",
            ...
            // Assert
            expect(mockMicrosoftClient.createAuthorizationURL).toHaveBeenCalledWith("mock-state-abc123", "mock-code-verifier-123", ["User.Read", "email", "profile", "openid"]);
        });
    });
...
//# sourceMappingURL=oauth.routes.test.js.map