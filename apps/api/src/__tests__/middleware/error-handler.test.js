...
describe("Error Handler Middleware (error-handler.ts)", () => {
    ...
    describe("Response format", () => {
        ...
        it("should only include details field when present", async () => {
            ...
            // Act - without details
            errorHandler(errorWithoutDetails, mockRequest, mockReply);
            const responseWithoutDetails = mockReply.send.mock.calls[0]?.[0];
            // Act - with details
            mockReply.send.mockClear();
            errorHandler(errorWithDetails, mockRequest, mockReply);
            const responseWithDetails = mockReply.send.mock.calls[0]?.[0];
            // Assert
    ...
    describe("Error type discrimination", () => {
        it("should correctly identify AppError subclasses", async () => {
            ...
            for (const error of appErrors) {
                ...
                const statusCall = mockReply.status.mock.calls[0]?.[0];
